...
export const mockElectronApi: ElectronAPI = {
  ...
  llm: {
    ...
    extract: vi
      .fn()
      .mockResolvedValue({ success: true, data: '', meta: { timestamp: Date.now() } }),
  ...
  templates: {
    load: vi
      .fn()
      .mockResolvedValue({ success: true, data: { version: 1, templates: [], categories: [] } }),
    save: vi.fn().mockResolvedValue({ success: true }),
  ...
  dialog: {
    showSaveDialog: vi
      .fn()
      .mockResolvedValue({ success: true, canceled: false, filePath: '/mock/save.json' }),
    showOpenDialog: vi
      .fn()
      .mockResolvedValue({ success: true, canceled: false, filePaths: ['/mock/open.json'] }),
  },
  artifact: {
  ...
  aiEditor: {
    generatePlan: vi
      .fn()
      .mockResolvedValue({ success: true, data: {}, meta: { timestamp: Date.now() } }),
    generatePlanWithAgent: vi
      .fn()
      .mockResolvedValue({ success: true, data: {}, meta: { timestamp: Date.now() } }),
    generatePlanStreaming: vi
      .fn()
      .mockResolvedValue({ success: true, message: 'Streaming started' }),
