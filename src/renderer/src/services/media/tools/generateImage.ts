...
export const generateImageTool: MediaToolDefinition = {
  name: 'generate_image',
  description:
    'Generate an image from a text prompt. Creates an artifact node with the result. Supports style presets and aspect ratios.',
  input_schema: {
    type: 'object',
    properties: {
      prompt: {
...
export async function executeGenerateImage(params: {
  ...
  const nodeId = store.addNode(
    ...
    { x: 0, y: 0 },
  ) // Position will be adjusted by spatial layout
