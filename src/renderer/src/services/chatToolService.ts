...
import { segmentsIntersect } from '../utils/layoutAlgorithms'
import { calculateAutoFitDimensions, TYPE_BADGE_H, MIN_BODY_H } from '../utils/nodeUtils'
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      try {
        ...
        console.log('[ChatToolService] run-layout event received. nodeIds:', nodeIds.length, 'edgeIds:', edgeIds.length, 'conversationId:', conversationId)
        // Log node dimensions before layout
        for (const nid of nodeIds) {
          const n = freshStore.nodes.find(nd => nd.id === nid)
          if (n) console.log(`  Node ${nid.slice(0,8)}: ${n.data.title?.slice(0,20)} w=${n.width ?? n.measured?.width} h=${n.height ?? n.measured?.height} content=${(n.data.content || '').length}chars type=${n.data.type} contentType=${n.data.contentType}`)
        }
  ...
  try {
    while (toolCallCount < TOOL_LOOP_SAFETY_LIMIT && !loopState.cancelled) {
      ...
      if (stopReason === 'tool_use' && currentToolUse) {
        ...
        const MUTATING_TOOLS = ['create_node', 'link_nodes', 'update_node', 'delete_node', 'unlink_nodes', 'move_node', 'add_comment', 'batch_create']
...
function runLayoutPipeline(
  ...
  for (const id of createdNodeIds) {
    ...
    const content = (node.data.type === 'artifact' && rawContent.length > 500)
    ...
    const isHtml = node.data.contentType === 'html' || (node.data.type === 'artifact' && rawContent.includes('<!DOCTYPE'))
    const contentFloor = isHtml
      ? Math.max(480, Math.min(1200, Math.round(rawContent.length * 0.9)))
      : rawContent.length > 1000 ? 500
      : rawContent.length > 500 ? 400
      : rawContent.length > 200 ? 300
      : 0
    const widthFloor = isHtml ? 680
      : rawContent.length > 300 ? 480
      : rawContent.length > 100 ? 340
      : 0
    const finalW = Math.max(w, dims.width, widthFloor)
    const finalH = Math.max(h, dims.height, minH, contentFloor)
    if (import.meta.env.DEV) console.log(`[Layout Step 1] Node ${id.slice(0,8)} "${(node.data.title || '').slice(0,20)}" type=${node.data.type} contentType=${node.data.contentType} content=${rawContent.length}chars w=${w}→${finalW} h=${h}→${finalH} isHtml=${isHtml} contentFloor=${contentFloor} widthFloor=${widthFloor}`)
    if (finalW > w || finalH > h) {
      store.updateNode(id, {
        width: finalW,
        height: finalH
      } as any, { skipHistory: true })
      if (import.meta.env.DEV) console.log(`[Layout Step 1] → RESIZED ${id.slice(0,8)} to ${finalW}x${finalH}`)
    }
  }
  ...
  switch (graphType) {
    ...
    case 'dag':
      positions = applyClusteredHierarchicalLayout(subNodes, subEdges, {
        nodeGap: 80, rankGap: 180, clusterGap: 200
      })
      break
    case 'chain':
      positions = applyHierarchicalLayout(subNodes, subEdges, 'LR', { nodeGap: 120, edgeLength: 200 })
      break
    case 'hub':
  ...
  const layoutTargetX = rightSpace >= leftSpace
    ? convX + convW + convOffset   // right side
    : convX - convOffset           // left side (layout extends leftward)

  let sumX = 0, sumY = 0, count = 0
  for (const pos of positions.values()) { sumX += pos.x; sumY += pos.y; count++ }
  if (count > 0) {
  ...
  for (const [edgeId, handles] of spreadAssignments) {
    ...
    if (edge && (edge.sourceHandle !== handles.sourceHandle || edge.targetHandle !== handles.targetHandle)) {
  ...
  for (let iter = 0; iter < 3; iter++) {
    ...
    const edgeLines = freshEdges.map(e => {
      ...
      const p1 = getHandlePosition({ x: srcPos.x, y: srcPos.y }, { width: srcPos.width, height: srcPos.height }, e.sourceHandle)
      const p2 = getHandlePosition({ x: tgtPos.x, y: tgtPos.y }, { width: tgtPos.width, height: tgtPos.height }, e.targetHandle)
    ...
    if (EDGE_ROUTING_DIAGNOSTIC) {
      console.log(`[EdgeRouting] Iteration ${iter}: ${iterCrossingCount} crossings detected, attempted=${attempted}`)
    }
