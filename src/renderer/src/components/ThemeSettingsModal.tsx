...
import type {
  AmbientEffectSettings as AmbientEffectSettingsType,
  AmbientEffectType,
  CustomThemePreset,
  EdgeStyle,
  FontTheme,
...
import { EFFECT_REGISTRY, EFFECTS_BY_CATEGORY } from './ambient/effectRegistry'
import {
  deriveColor,
  generatePaletteFromAccents,
  hexToRgbFloat,
} from './ambient/utils/colorConvert'
...
function AmbientEffectSettingsComponent({
  ...
  const resolvedValues = (() => {
    ...
    const baseColor =
      accentColor && accentColor.startsWith('#') && accentColor.length >= 4
        ? accentColor
        : '#8b5cf6'
    ...
    for (const propKey of entry.themeColorProps) {
      ...
      if (schema.controlType === 'color-array') {
        ...
        if (schema.colorFormat === 'hex') {
          ...
          resolved[propKey] = generatePaletteFromAccents(baseColor, accentSecondary, count).map(
            (h) => hexToRgbFloat(h),
          )
        }
    ...
    <div className="space-y-3">
      ...
      <button
        ...
        className={`px-3 py-1 rounded text-xs transition-all ${
          !isEffectSelected ? 'gui-bg-accent text-white' : `${buttonBg} ${textSecondary}`
        }`}
      ...
      <div className="space-y-2">
        {EFFECTS_BY_CATEGORY.map(({ category, effects }) => (
          <div key={category}>
            <h4 className={`text-[10px] ${textMuted} uppercase tracking-wider mb-1`}>{category}</h4>
      ...
      {isEffectSelected && (
        <div className="space-y-2 pl-1">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <span className={`text-xs ${textMuted} w-8 text-right`}>
              {settings.bloomIntensity ?? 30}%
            </span>
          </div>
...
function ThemeSettingsModalComponent({ open, onOpenChange }: ThemeSettingsModalProps): JSX.Element {
  ...
  const [genericColorPickerTarget, setGenericColorPickerTarget] = useState<
    'savedColors' | 'canvasBg' | 'canvasGrid' | null
  >(null)
  ...
  const handleGenericColorPickerChange = useCallback(
    (color: string): void => {
  ...
  const currentGuiColors: GuiColors =
    themeSettings.guiColors || (isLightMode ? DEFAULT_GUI_LIGHT : DEFAULT_GUI_DARK)

  const handleOpenGuiColorPicker = useCallback(
    (target: string): void => {
  ...
  const handleQuickSetGuiColor = useCallback(
    (target: string, color: string): void => {
      const newColors = { ...currentGuiColors }
  ...
  const handleGenerateAiTheme = useCallback(
...
Output ONLY the JSON object with nodeColors, canvas colors, guiColors, and linkColors for both dark and light modes.`
        ...
        if (
          !generatedTheme.nodeColors ||
          !generatedTheme.canvasBackgroundDark ||
          !generatedTheme.canvasBackgroundLight
        ) {
        ...
        if (currentMode === 'dark') {
          ...
          if ((generatedTheme as Record<string, unknown>).linkColorsDark) {
            setLinkColors(
              (generatedTheme as Record<string, unknown>).linkColorsDark as {
                default: string
                active: string
                inactive: string
                selected: string
          ...
          if ((generatedTheme as Record<string, unknown>).linkColorsLight) {
            setLinkColors(
              (generatedTheme as Record<string, unknown>).linkColorsLight as {
                default: string
                active: string
                inactive: string
                selected: string
        ...
        useWorkspaceStore.setState({
          ...
          themeSettings: {
            ...
            guiColorsDark: (generatedTheme as Record<string, unknown>).guiColorsDark as
              | GuiColors
              | undefined,
            guiColorsLight: (generatedTheme as Record<string, unknown>).guiColorsLight as
              | GuiColors
              | undefined,
            // Store per-mode link colors
            linkColorsDark: (generatedTheme as Record<string, unknown>).linkColorsDark as
              | { default: string; active: string; inactive: string; selected: string }
              | undefined,
            linkColorsLight: (generatedTheme as Record<string, unknown>).linkColorsLight as
              | { default: string; active: string; inactive: string; selected: string }
              | undefined,
          },
        })
  ...
  const handleRevertAiTheme = useCallback((): void => {
    ...
    aiPreview.originalTheme,
    aiPreview.generatedTheme,
    setThemeColor,
    setCanvasBackground,
    setCanvasGridColor,
    deleteCustomPreset,
  ...
  const handleRegenerateAiTheme = useCallback(
    (additionalPrompt?: string): void => {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        <SheetHeader>
          ...
          <SheetDescription>Customize colors, effects, and appearance</SheetDescription>
        </SheetHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div
              className={`flex items-center gap-1.5 text-xs font-medium ${textSecondary} uppercase`}
            >
              Mode
            </div>
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded transition-colors ${
                  !isLightMode ? 'gui-ring-active' : `${buttonBg} ${textSecondary}`
                }`}
                style={
                  !isLightMode
                    ? {
                        backgroundColor:
                          'color-mix(in srgb, var(--gui-accent-primary) 20%, var(--gui-panel-bg-secondary))',
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {(Object.keys(FONT_THEMES) as FontTheme[]).map((key) => {
                ...
                return (
                  ...
                  >
                    <span className="text-xs gui-text block" style={{ fontFamily: font.display }}>
                      {font.label}
                    </span>
                    <span
                      className="text-xs gui-text-secondary block mt-0.5"
          ...
          <div className="space-y-2">
            <div
              className={`flex items-center justify-between text-xs font-medium ${textSecondary} uppercase`}
            >
              <span>Font Size</span>
              <span className="text-xs font-mono normal-case">
                {themeSettings.fontSize || 14}px
              </span>
            </div>
            <input
              type="range"
                ...
                if (preset) {
                    ...
                    <div key={preset.id} className="group relative">
                      ...
                      >
                        ...
                        <div className="flex gap-0.5 justify-center">
                          {Object.values(preset.nodeColors)
                            .slice(0, 6)
                            .map((color, i) => (
                              <div
                                key={i}
                        ...
                        {/* Preset name */}
                        <div
                          className={`text-[10px] mt-0.5 text-center truncate text-[var(--text-muted)]`}
                        >
                          {preset.name}
                        </div>
                      </button>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`text-[10px] text-center ${textMuted}`}>Empty</div>
                    </button>
                  )
                }
              })}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {aiPreview.isGenerating ? (
                  ...
                  <>
                    ...
                    AI Generate
                    {!hasEmptyPresetSlot && themeSettings.aiPaletteEnabled && (
                      <span className="text-[8px] ml-1">(need slot)</span>
                    )}
                  </>
                )}
              </button>
              <button
                ...
                className={`relative w-8 h-4 rounded-full transition-colors flex-shrink-0 ${
                  themeSettings.aiPaletteEnabled
                    ? 'gui-bg-accent'
                    : 'bg-[var(--surface-panel-secondary)]'
                }`}
              ...
              <div className="flex gap-1 flex-1">
                {gridPresets.map(({ color, label }) => (
                  <button
                    ...
                    style={{
                      backgroundColor:
                        color === '#transparent' ? (isLightMode ? '#f5f5f5' : '#1a1a2e') : color,
                    }}
                    title={label}
                  >
                ...
                <div key={type} className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      <span
                        className={`text-xs text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]`}
                      >
                        {label}
                      </span>
                    </button>
                    ...
                    <div className="pl-5 space-y-2">
                      ...
                      <div className="flex gap-1 items-center flex-wrap">
                        ...
                        {/^#[0-9A-Fa-f]{6}$/.test(customInput) &&
                          !customColors.includes(customInput) && (
                            <button
                              onClick={() => {
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className={`p-1 ${buttonBg} rounded transition-colors`}
                    title="Close (Cancel)"
          ...
          >
              ...
              {!themeSettings.linkGradientEnabled && (
                <div className="space-y-2">
                  ...
                  <div className="space-y-1.5">
                    <button
                      onClick={() =>
                        setExpandedLinkSection(expandedLinkSection === 'default' ? null : 'default')
                      }
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1">
                        <div
                          ...
                          style={{
                            backgroundColor: (
                              themeSettings.linkColors ||
                              (isLightMode ? DEFAULT_LINK_COLORS_LIGHT : DEFAULT_LINK_COLORS_DARK)
                            ).default,
                          }}
                        />
                  ...
                  <div className="space-y-1.5">
                    <button
                      onClick={() =>
                        setExpandedLinkSection(expandedLinkSection === 'active' ? null : 'active')
                      }
                      className={`flex items-center justify-between w-full text-left`}
                    >
                  ...
                  <div className="space-y-1.5">
                    <button
                      onClick={() =>
                        setExpandedLinkSection(
                          expandedLinkSection === 'inactive' ? null : 'inactive',
                        )
                      }
                      className={`flex items-center justify-between w-full text-left`}
                    >
                  ...
                  <div className="space-y-1.5">
                    <button
                      onClick={() =>
                        setExpandedLinkSection(
                          expandedLinkSection === 'selected' ? null : 'selected',
                        )
                      }
                      className={`flex items-center justify-between w-full text-left`}
                    >
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-medium text-sm ${textPrimary}`}>
                        {linkColorTarget.charAt(0).toUpperCase() + linkColorTarget.slice(1)} Link
                        Color
                      </span>
                    </div>
                    <button
                      ...
                      className={`p-1 ${buttonBg} rounded transition-colors`}
                      title="Close"
                      aria-label="Close link color picker"
                    >
                      <X className={`w-4 h-4 ${textSecondary}`} />
                    </button>
                  </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                <button
                  onClick={() =>
                    setExpandedGuiSection(
                      expandedGuiSection === 'backgrounds' ? null : 'backgrounds',
                    )
                  }
                  className={`flex items-center justify-between w-full text-left`}
                >
                  ...
                  <div className="flex items-center gap-1">
                    <div
                      className="w-4 h-4 rounded-full"
                      style={{ backgroundColor: currentGuiColors.panelBackground }}
                      title="Primary"
                    />
                    <div
                      className="w-4 h-4 rounded-full"
                      style={{ backgroundColor: currentGuiColors.panelBackgroundSecondary }}
                      title="Secondary"
                    />
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <div
                          ...
                          style={{
                            borderColor: 'var(--gui-accent-primary)',
                            backgroundColor: currentGuiColors.panelBackground,
                          }}
                        />
                      </div>
                      <div className="flex flex-wrap gap-0.5 pl-16">
                        {/* Secondary palette - neutral/grayscale backgrounds */}
                        {(isLightMode ? BACKGROUND_PALETTE.light : BACKGROUND_PALETTE.dark).map(
                          (color, i) => (
                            <button
                              key={`bg-preset-${i}`}
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <div
                          ...
                          style={{
                            borderColor: 'var(--gui-accent-primary)',
                            backgroundColor: currentGuiColors.panelBackgroundSecondary,
                          }}
                        />
                      </div>
                      <div className="flex flex-wrap gap-0.5 pl-16">
                        {(isLightMode ? BACKGROUND_PALETTE.light : BACKGROUND_PALETTE.dark).map(
                          (color, i) => (
                            <button
                              key={`bg2-preset-${i}`}
                              onClick={() =>
                                handleQuickSetGuiColor('panelBackgroundSecondary', color)
                              }
              ...
              <div className="space-y-1.5">
                <button
                  onClick={() =>
                    setExpandedGuiSection(expandedGuiSection === 'text' ? null : 'text')
                  }
                  className={`flex items-center justify-between w-full text-left`}
                >
                  ...
                  <div className="flex items-center gap-1">
                    <div
                      className="w-4 h-4 rounded-full"
                      style={{ backgroundColor: currentGuiColors.textPrimary }}
                      title="Primary"
                    />
                    <div
                      className="w-4 h-4 rounded-full"
                      style={{ backgroundColor: currentGuiColors.textSecondary }}
                      title="Secondary"
                    />
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <div
                          ...
                          style={{
                            borderColor: 'var(--gui-accent-primary)',
                            backgroundColor: currentGuiColors.textPrimary,
                          }}
                        />
                      </div>
                      <div className="flex flex-wrap gap-0.5 pl-16">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <div
                          ...
                          style={{
                            borderColor: 'var(--gui-accent-primary)',
                            backgroundColor: currentGuiColors.textSecondary,
                          }}
                        />
                      </div>
                      <div className="flex flex-wrap gap-0.5 pl-16">
              ...
              <div className="space-y-1.5">
                <button
                  onClick={() =>
                    setExpandedGuiSection(expandedGuiSection === 'accents' ? null : 'accents')
                  }
                  className={`flex items-center justify-between w-full text-left`}
                >
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <div
                          ...
                          style={{
                            borderColor: 'var(--gui-accent-primary)',
                            backgroundColor: currentGuiColors.accentPrimary,
                          }}
                        />
                      </div>
                      <div className="flex flex-wrap gap-0.5 pl-16">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <div
                          ...
                          style={{
                            borderColor: 'var(--gui-accent-primary)',
                            backgroundColor: currentGuiColors.accentSecondary,
                          }}
                        />
                      </div>
                      <div className="flex flex-wrap gap-0.5 pl-16">
              ...
              <div className="space-y-1.5">
                <button
                  onClick={() =>
                    setExpandedGuiSection(expandedGuiSection === 'toolbar' ? null : 'toolbar')
                  }
                  className={`flex items-center justify-between w-full text-left`}
                >
                  ...
                  <div className="flex items-center gap-1">
                    <div
                      className="w-4 h-4 rounded-full"
                      style={{ backgroundColor: currentGuiColors.toolbarIconDefault }}
                      title="Default"
                    />
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <div
                          ...
                          style={{
                            borderColor: 'var(--gui-accent-primary)',
                            backgroundColor: currentGuiColors.toolbarIconDefault,
                          }}
                        />
                      </div>
                      <div className="flex flex-wrap gap-0.5 pl-16">
                      ...
                      <div key={idx} className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <div
                            ...
                            style={{
                              borderColor: 'var(--gui-accent-primary)',
                              backgroundColor: currentGuiColors.toolbarIconAccent[idx],
                            }}
                          />
                        </div>
                        <div className="flex flex-wrap gap-0.5 pl-16">
                          {ALL_COLORS.map((color, i) => (
                            <button
                              ...
                              onClick={() =>
                                handleQuickSetGuiColor(`toolbarIconAccent.${idx}`, color)
                              }
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-medium text-sm ${textPrimary}`}>
                        ...
                        {guiColorTarget.startsWith('toolbarIconAccent') &&
                          `Toolbar Icon ${parseInt(guiColorTarget.split('.')[1] ?? '0') + 1}`}
                      </span>
                    </div>
                    <button
                      ...
                      className={`p-1 ${buttonBg} rounded transition-colors`}
                      title="Close"
                      aria-label="Close GUI color picker"
                    >
                      <X className={`w-4 h-4 ${textSecondary}`} />
                    </button>
                  </div>
            ...
            <div className="space-y-2">
              ...
              {customColors.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {customColors.map((color) => (
                    <div key={color} className="group relative">
                      <div
                        className={`w-6 h-6 rounded-full border-2 cursor-pointer transition-all ${'border-[var(--border-subtle)] hover:border-[var(--border-default)]'}`}
                        style={{ backgroundColor: color }}
                        title={color}
                      />
                      <button
                ...
                <p className={`text-xs ${textMuted} italic`}>
                  No saved colors. Click "Pick" to add colors.
                </p>
              )}
              ...
              {customColors.length > 0 && (
                <p className={`text-xs ${textMuted}`}>
                  Click a node color row above, then select from saved colors.
                </p>
              )}
            </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col gap-0.5">
                  ...
                  <span className={`text-[10px] ${textMuted}`}>
                    Nodes adopt project color when added
                  </span>
                </div>
                <button
                  onClick={() => {
                  ...
                  className={`w-8 h-4 rounded-full transition-colors ${
                    themeSettings.inheritParentStyle !== false
                      ? 'gui-bg-accent'
                      : 'bg-[var(--surface-panel-secondary)]'
                  }`}
                >
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    onClick={() => setShowAiGenerateModal(false)}
                    className={`p-1 ${buttonBg} rounded transition-colors`}
                    aria-label="Close AI theme generator"
                  >
                    <X className={`w-4 h-4 ${textSecondary}`} />
                  </button>
                </div>
          ...
          {aiPreview.isActive && (
            <div
              className={`absolute bottom-0 left-0 right-0 ${panelBg} border-t ${panelBorder} p-3 shadow-lg z-[61]`}
            >
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs ${textMuted} truncate flex-1`}>
                    "{aiPreview.description}"
                  </span>
                </div>
