...
function CanvasDistrictOverlayComponent(): JSX.Element | null {
  ...
  const districts = useMemo(() => regions.filter((r) => r.isDistrict), [regions])
  ...
  return (
    <div className="absolute inset-0 pointer-events-none" style={{ zIndex: -1 }}>
      {districts.map((district) => (
        <DistrictRect key={district.id} region={district} viewport={viewport} />
      ))}
    </div>
  )
}
...
const DistrictRect = memo(function DistrictRect({
  region,
  viewport,
}: DistrictRectProps): JSX.Element {
