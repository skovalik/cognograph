...
// EdgeGrammarLegend — Connection Grammar Legend
    ...
    items: [
      {
        visual: 'arrow',
        label: 'Directed',
        desc: 'Information flows one way',
        strokeWidth: 2.5,
        dash: undefined,
        opacity: 0.9,
        arrow: true,
          ...
          <div key={group.category} className="edge-grammar-legend__group">
            ...
            {group.items.map((item) => (
              <div key={item.label} className="edge-grammar-legend__item">
                <svg
                  className="edge-grammar-legend__swatch"
                  width="40"
                  height="16"
                  viewBox="0 0 40 16"
                >
                  <line
                    x1="2"
                    y1="8"
                    x2={item.arrow || item.bidir ? 32 : 38}
                    y2="8"
                  ...
                  {item.arrow && (
                    <polygon
                      points="32,4 38,8 32,12"
                      fill="var(--gui-text-secondary)"
                      opacity={item.opacity}
                    />
                  )}
                  {item.bidir && (
