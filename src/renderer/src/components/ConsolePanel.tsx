...
import { ConsoleLogEntry, useConsoleLogStore } from '../stores/consoleLogStore'
        ...
        <button
          onClick={() => {
            const text = entries
              .map(
                (e) =>
                  `[${new Date(e.timestamp).toLocaleTimeString('en-US', { hour12: false })}] [${e.source}] ${e.message}`,
              )
              .join('\n')
      ...
      >
        {filtered.map((entry) => (
          <div
            ...
            className="flex gap-2 leading-tight py-0.5 hover:bg-white/5 rounded px-1"
          >
            ...
            <span
              style={{ color: LEVEL_COLORS[entry.level] || 'var(--gui-text-secondary)' }}
