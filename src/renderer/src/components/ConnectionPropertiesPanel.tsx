...
import { Checkbox, ScrollArea, Separator } from './ui'
...
export function ConnectionPropertiesPanel({
  edgeId,
  onClose,
}: ConnectionPropertiesPanelProps): JSX.Element {
  ...
  return (
    <div className="w-80 gui-panel glass-soft gui-border flex flex-col h-full">
      ...
      <div className="flex items-center justify-between p-4">
        <h2 className="font-semibold gui-text">Connection Properties</h2>
        <button onClick={onClose} className="p-1 gui-button rounded transition-colors">
        ...
        {isMultiSelect ? (
          ...
          <>
            ...
            <div className="flex items-center gap-2 text-sm">
              <span
                className="gui-text truncate max-w-[100px]"
                title={sourceNode?.data.title as string | undefined}
              >
              ...
              <span
                className="gui-text truncate max-w-[100px]"
                title={targetNode?.data.title as string | undefined}
              >
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-6">
              ...
              {edgeData.direction === 'unidirectional' && (
                <div className="mt-3">
                  ...
                  <p className="text-xs gui-text-secondary mt-1 text-center">
                    Swap {(sourceNode?.data.title as string) || 'source'} ↔{' '}
                    {(targetNode?.data.title as string) || 'target'}
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="flex gap-2 mb-2">
                <button
                  ...
                  className={`flex items-center justify-center w-9 h-9 rounded border transition-colors ${
                    edgeData.labelBold ? 'gui-ring-active' : 'gui-border gui-button'
                  }`}
                  style={
                    edgeData.labelBold
                      ? {
                ...
                <button
                  ...
                  className={`flex items-center justify-center w-9 h-9 rounded border transition-colors ${
                    edgeData.labelItalic ? 'gui-ring-active' : 'gui-border gui-button'
                  }`}
                  style={
              ...
              <div className="flex flex-wrap gap-1">
                <button
                  ...
                  className={`px-2 py-1 text-xs rounded transition-colors border ${
                    !edgeData.label ? 'gui-ring-active' : 'gui-border gui-button'
                  }`}
                  style={
                    !edgeData.label
                      ? {
                ...
                {EDGE_LABEL_PRESETS.map((preset) => (
                  <button
                    ...
                    className={`px-2 py-1 text-xs rounded transition-colors border ${
                      edgeData.label === preset ? 'gui-ring-active' : 'gui-border gui-button'
                    }`}
                    style={
          ...
          {/* Connection Strength */}
          <div>
            <label className="block text-sm font-medium gui-text mb-2">Connection Strength</label>
            <div className="flex gap-2">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              <button
                ...
                className={`px-2 py-1 text-xs rounded transition-colors border ${
                  isMultiSelect
                    ? sharedValues?.edgeStyle === undefined
                      ? 'gui-ring-active'
                      : 'gui-border gui-button'
                    : !edgeData.edgeStyle
                      ? 'gui-ring-active'
                ...
                style={
                  (isMultiSelect ? sharedValues?.edgeStyle === undefined : !edgeData.edgeStyle)
                    ? {
                        backgroundColor:
                          'color-mix(in srgb, var(--gui-accent-primary) 20%, transparent)',
                        color: 'var(--gui-accent-primary)',
            ...
            <p className="text-xs gui-text-secondary mt-1">
              {isMultiSelect
                ? 'Apply style to all selected connections'
                : 'Override global edge style for this connection'}
            </p>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-1">
              {ARROW_STYLES.map((style) => {
                const currentValue = isMultiSelect
                  ? sharedValues?.arrowStyle
                  : edgeData.arrowStyle || 'filled'
                const isSelected = currentValue === style.value
                const isMixed = isMultiSelect && currentValue === 'mixed'
          ...
          <div>
            ...
            <div className="flex gap-1">
              {STROKE_PRESETS.map((preset) => {
                const currentValue = isMultiSelect
                  ? sharedValues?.strokePreset
                  : edgeData.strokePreset || 'normal'
              ...
              {isMultiSelect ? (
                ...
                <>
                  <div className="text-xs gui-text-secondary">
                    Total waypoints across {edgeIds.length} connections:{' '}
                    {sharedValues?.totalWaypoints || 0}
                  </div>
                ...
                <>
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-xs gui-text-secondary">
                      <span>Waypoints:</span>
                      <span
                        className={`font-mono ${(edgeData.waypoints?.length || 0) >= 20 ? 'text-amber-500 font-semibold' : (edgeData.waypoints?.length || 0) >= 15 ? 'text-yellow-500' : ''}`}
                      >
                        {edgeData.waypoints?.length ||
                          (edgeData.centerOffset &&
                          (Math.abs(edgeData.centerOffset.x) > 5 ||
                            Math.abs(edgeData.centerOffset.y) > 5)
                            ? 1
                            : 0)}
                        /20
                  ...
                  (edgeData.centerOffset &&
                    ...
                    <button
                      onClick={() =>
                        handleUpdate({ waypoints: undefined, centerOffset: { x: 0, y: 0 } })
                      }
                      className="w-full flex items-center justify-center gap-2 px-3 py-2 gui-button rounded text-sm transition-colors"
                    >
                      <RotateCcw className="w-3.5 h-3.5" />
                  ...
                  <div className="text-xs gui-text-secondary space-y-1 pt-1">
                    ...
                    <p>
                      •{' '}
                      <kbd className="px-1 py-0.5 bg-[var(--surface-panel-secondary)] rounded text-[10px]">
                        Shift
                      </kbd>{' '}
                      + drag to snap to grid
                    </p>
                    <p>
                      •{' '}
                      <kbd className="px-1 py-0.5 bg-[var(--surface-panel-secondary)] rounded text-[10px]">
      ...
      <div className="p-4">
        ...
        >
          ...
          <span>
            {isMultiSelect ? `Delete ${edgeIds.length} Connections` : 'Delete Connection'}
          </span>
        </button>
      </div>
