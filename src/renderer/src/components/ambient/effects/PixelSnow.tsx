    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        if (dist < flakeSize) {
          ...
          if (uIsDark == 1) {
            // Dark mode: premultiplied alpha — flakes fade to transparent (→ dark bg)
            gl_FragColor = vec4(uColor * a, a);
          } else {
            // Light mode: opaque color fade — flakes fade toward white (→ blends with light bg)
            gl_FragColor = vec4(mix(vec3(1.0), uColor, a), a);
          }
          return;
        }
      }
