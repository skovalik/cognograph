...
const Beams: FC<BeamsProps> = ({
  ...
  vec3 getNormal(vec3 pos) {
    ...
    if (uIsDark == 0) {
      float luma = clamp(dot(gl_FragColor.rgb, vec3(0.299, 0.587, 0.114)), 0.0, 1.0);
      gl_FragColor.rgb = mix(vec3(1.0), uLightColor, sqrt(luma));
    }`
        },
        material: { fog: true },
        uniforms: {
          ...
          uScale: scale,
          uIsDark: isDark ? 1 : 0,
          uLightColor: new THREE.Color(...hexToNormalizedRGB(lightColor))
        }
      }),
    [speed, noiseIntensity, scale, isDark, lightColor]
  );
  ...
  return (
    <CanvasWrapper>
      ...
      {/* Transparent — composites over canvas background */}
