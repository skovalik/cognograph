      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {template.builtIn && <Star className="w-3 h-3 text-yellow-500 flex-shrink-0" />}
          </div>
  ...
  return (
    <div>
      ...
      {variable.description && (
        <p className="text-[10px] mt-1 text-[var(--text-secondary)]">{variable.description}</p>
      )}
    </div>
  )
})
...
function WorkflowTemplateSelectorComponent({
  ...
  const canApply =
    selectedTemplate &&
    selectedTemplate.variables.filter((v) => v.required).every((v) => variableValues[v.name])
        ...
        <div className="flex-1 overflow-auto">
          ...
          <div className="p-4 pb-2">
            <div className="relative">
              <Search
                className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${textMutedClass}`}
              />
              <input
                type="text"
                value={search}
          ...
          <div className="px-4 pb-4 space-y-2">
            {filteredTemplates.length === 0 ? (
              <p className={`text-sm text-center py-8 ${textMutedClass}`}>No templates found</p>
            ) : (
